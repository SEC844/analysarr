...
> = {
  no_torrent: {
    label: 'No torrent',
    description: 'File present in library but no matching torrent in qBittorrent',
    color: 'amber',
    icon: AlertTriangle,
  },
  orphan_torrent: {
    label: 'Orphan torrent',
    description: 'Torrent exists in qBittorrent but not linked to any *arr entry',
    color: 'red',
    icon: Ghost,
  },
  duplicate: {
    label: 'Duplicate',
    description: 'Multiple torrents matched to the same media entry',
    color: 'amber',
    icon: Copy,
  },
  copy_not_hardlink: {
    label: 'Not hardlinked',
    description: 'File appears to be a copy rather than a hardlink',
    color: 'red',
    icon: Link2,
  },
};
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-xl border border-zinc-800 bg-zinc-900/50 py-20 text-center">
          <div className="mb-3 rounded-full bg-green-900/30 p-4">
            <AlertTriangle className="h-8 w-8 text-green-400" />
          </div>
          <p className="font-medium text-white">No issues detected</p>
          <p className="mt-1 text-sm text-zinc-500">Your library looks healthy!</p>
        </div>
      ) : (
              ...
              return (
                <section key={type} className="space-y-3">
                  <div className="flex items-center gap-2">
                    <Icon className={`h-4 w-4 ${iconColorMap[meta.color]}`} />
                  ...
                  <ul className="space-y-2">
                    {items.map((issue) => (
                      <li
                        key={issue.id}
                        className={`flex items-start gap-3 rounded-xl border p-4 ${colorMap[meta.color]}`}
                      >
                        <div className="mt-0.5 shrink-0">
                          {issue.mediaType === 'movie' ? (
                            <Film className="h-4 w-4" />
                          ) : issue.mediaType === 'series' ? (
                        ...
                        <div className="min-w-0">
                          ...
                          {issue.torrentHash && (
                            <p className="text-xs opacity-50 mt-0.5 font-mono truncate">
                              {issue.torrentHash}
                            </p>
                          )}
                        </div>
